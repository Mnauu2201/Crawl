# """Facebook Crawler: Quét reel/video từ fanpage + tải về."""
# import asyncio, subprocess, traceback, threading, re as _re
...
#     # ── Chuyển storage_state.json → cookies.txt (Netscape) cho yt-dlp ──
#     @staticmethod
#     def _storage_state_to_netscape(json_path):
#         import json as _json, time as _time
#         data = _json.loads(Path(json_path).read_text(encoding="utf-8"))
#         lines = ["# Netscape HTTP Cookie File"]
#         far_future = int(_time.time()) + 60 * 60 * 24 * 365
#         for c in data.get("cookies", []):
#             domain = c.get("domain", "")
...
#     @staticmethod
#     def _cookies_for_ytdlp(cookies_file):
#         """Trả về đường dẫn cookies.txt hợp lệ cho yt-dlp (tự convert nếu là .json)."""
#         if not cookies_file or not Path(cookies_file).exists():
#             return None
#         if str(cookies_file).lower().endswith(".json"):
#             try:
#                 return FBCrawler._storage_state_to_netscape(cookies_file)
#             except Exception:
#                 return None
...
#                     # Chờ og:description xuất hiện (tối đa 5s) trước khi extract.
#                     try:
...
#                     # Retry một lần nếu chưa lấy được
#                     if not title:
...
#         await asyncio.gather(*[_one(v) for v in need])
...
#     # ── Sổ theo dõi reel đã tải (lưu theo thư mục output) ──────────
#     # Giúp lần quét sau biết reel nào đã tải/đã xử lý rồi, reel nào
#     # còn mới — vì crawl luôn quét lại từ video mới nhất trên cùng.
#     @staticmethod
#     def _registry_path(out_dir):
#         return Path(out_dir) / "_da_tai.json"

#     @staticmethod
#     def _load_registry(out_dir):
#         p = FBCrawler._registry_path(out_dir)
#         if p.exists():
#             try:
#                 import json as _json
...
#     @staticmethod
#     def _save_registry(out_dir, ids_set):
#         try:
#             import json as _json
#             p = FBCrawler._registry_path(out_dir)
#             p.parent.mkdir(parents=True, exist_ok=True)
#             p.write_text(_json.dumps(sorted(ids_set), ensure_ascii=False),
#                          encoding="utf-8")
#         except Exception:
#             pass
...
#                 if out_dir:
#                     done_ids = FBCrawler._load_registry(out_dir)
#                     n_marked = 0
#                     for v in videos:
#                         if FBCrawler._reel_id_from_url(v["url"]) in done_ids:
#                             v["downloaded"] = True
#                             n_marked += 1
#                         else:
#                             v["downloaded"] = False
...
#     # ── Tải video đã chọn ──────────────────────────────────────────
#     def download(self, items, out_dir, cookies_file):
#         n = len(items); ok = 0
...
#             self.log(f"\n[{i}/{n}] {title[:55]}", TEXT)
#             self.progress((i-1)/n, f"Tải {i}/{n}...")
#             safe = ("".join(c for c in title
#                             if c.isalnum() or c in "_- ")[:40]
#                     or f"reel_{i}")
#             out_path = out_dir / f"{safe}.mp4"
#             ytdlp = get_ytdlp()
#             cmd = ytdlp + ["-f", "best[ext=mp4]/best",
#                            "-o", str(out_path), url]
#             ck = FBCrawler._cookies_for_ytdlp(cookies_file)
#             if ck:
#                 cmd += ["--cookies", ck]
#             r = subprocess.run(cmd, capture_output=True, text=True)
#             if r.returncode == 0:
#                 self.log(f"  OK {out_path.name}", SUCCESS); ok += 1
#                 # Ghi nhận reel này đã tải xong vào sổ theo dõi
#                 done_ids.add(FBCrawler._reel_id_from_url(url))
#                 FBCrawler._save_registry(out_dir, done_ids)
#             else:
#                 self.log(f"  Lỗi: {r.stderr[-120:]}", ERR)
#         self.log(f"\nTải xong: {ok}/{n} video",
